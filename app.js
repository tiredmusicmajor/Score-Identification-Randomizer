...
const el = {
  ...
  revealPageWrap: document.querySelector('.reveal-page-wrap'),
  ...
  lbFullscreen: document.getElementById('lb-fullscreen'),
  lbPrev: document.getElementById('lb-prev'),
  lbNext: document.getElementById('lb-next'),
  lbIndicator: document.getElementById('lb-indicator'),
...
async function advanceToNextFile() {
  ...
  updateProgress();
  showScreen('loading');
  el.loadingText.textContent = 'Opening the score\u2026';
  ...
  try {
    ...
    console.error(err);
    el.loadingText.textContent = 'Couldn\u2019t read that file \u2014 skipping it.';
...
const lb = { pdf: null, pages: [], idx: 0 };

async function renderPageFitted(pdf, pageNumber, canvas, maxWidth, maxHeight) {
  const page = await pdf.getPage(pageNumber);
...
async function openLightbox(pdf, pages, startIdx) {
  lb.pdf = pdf;
  lb.pages = pages;
  lb.idx = startIdx;
  ...
  el.lbFullscreen.classList.toggle('hidden', !document.fullscreenEnabled);
...
async function renderLightboxPage() {
  ...
  await renderPageFitted(lb.pdf, lb.pages[lb.idx], el.lbCanvas, maxWidth, maxHeight);
  el.lbIndicator.textContent = lb.pages.length > 1
    ? `${lb.idx + 1} of ${lb.pages.length}`
    : '';
  el.lbPrev.disabled = lb.idx === 0;
  el.lbNext.disabled = lb.idx === lb.pages.length - 1;
}
...
el.lbFullscreen.addEventListener('click', () => {
  if (document.fullscreenElement) {
    document.exitFullscreen().catch(() => {});
  } else {
    el.lightbox.requestFullscreen?.().catch(() => {});
  }
});
...
async function showQuiz() {
  ...
  const excerptPageNumbers = Array.from({ length: count }, (_, i) => start + i);
  ...
  for (let i = 0; i < count; i++) {
    ...
    card.addEventListener('click', () => openLightbox(state.currentPdf, excerptPageNumbers, i));
...
el.revealPageWrap.addEventListener('click', () => {
  if (state.currentPdf) openLightbox(state.currentPdf, [1], 0);
});
